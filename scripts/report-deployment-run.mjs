...
const callbackStatus = process.env.DEPLOYMENT_CALLBACK_STATUS ?? "completed";
const conclusion = process.env.DEPLOYMENT_CALLBACK_CONCLUSION;
const phase = process.env.DEPLOYMENT_CALLBACK_PHASE ?? "deployment_completed";
...
const deploymentRunId = body?.data?.id;
if (typeof deploymentRunId !== "string" || !deploymentRunId.trim()) {
	fail("deployment run 响应缺少精确 deployment run ID，拒绝继续 smoke。");
}
const outputPath = process.env.GITHUB_OUTPUT;
if (outputPath) {
	await appendFile(outputPath, `deployment-run-id=${deploymentRunId}\n`);
}
