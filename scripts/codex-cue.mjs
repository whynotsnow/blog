...
const cueBin = "/Users/whynotsnow/.codex/bin/codex-cue";
...
function runCue(args) {
	...
	if (result.error) {
		fail(`unable to run ${cueBin}: ${result.error.message}`);
	}
...
function setEvent(event, optionArgs) {
	...
	if (Object.keys(options).length === 0) {
		fail(
			"set requires at least one option, such as --preset hero or --volume 3",
		);
	}

	const config = readConfig();
	const events =
		typeof config.events === "object" && config.events ? config.events : {};
	const current =
		typeof events[event] === "object" && events[event] ? events[event] : {};
...
function ensureCueBin() {
	try {
		...
		fail(`${cueBin} is missing or not executable`);
	}
}
