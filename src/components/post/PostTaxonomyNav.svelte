<script lang="ts">
	...
	const categorySlug = generateCategorySlug(currentCategory || "");
	const tagSlug = generateTagSlug(currentTag || "");
			...
			{#each categories as cat}
				...
				>
					<a
						...
						class:hover:bg-[var(--btn-regular-bg-hover)]={!isActiveCategory(
							cat.slug,
						)}
						aria-current={isActiveCategory(cat.slug)
							? "page"
							: undefined}
					>
						{cat.name}
                   ...
                   transition"
						class:bg-[var(--primary)]={isActiveCategory(cat.slug)}
						class:bg-[var(--btn-regular-bg)]={!isActiveCategory(
							cat.slug,
						)}
						class:group-hover:bg-[var(--btn-regular-bg-hover)]={!isActiveCategory(
							cat.slug,
						)}
					>
						{cat.count}
					...
					{#each activeCategory.tags as tag}
						...
						>
							<a
								...
								class:bg-[var(--primary)]={isActiveTag(
									tag.slug,
								)}
								class:pointer-events-none={isActiveTag(
									tag.slug,
								)}
								class:hover:bg-[var(--btn-regular-bg-hover)]={!isActiveTag(
									tag.slug,
								)}
								aria-current={isActiveTag(tag.slug)
									? "page"
									: undefined}
							>
								# {tag.name}
                       ...
                       transition"
								...
								class:bg-[var(--btn-regular-bg)]={!isActiveTag(
									tag.slug,
								)}
								class:group-hover:bg-[var(--btn-regular-bg-hover)]={!isActiveTag(
									tag.slug,
								)}
							>
								{tag.count}
