<script lang="ts">
  ...
  import type { PostMeta } from "./useCategoryPagination";
  ...
  onMount(async () => {
    // ⚠️ 不能再使用 Astro inline script
    // 必须使用 onMount 执行客户端逻辑
...
>
  ...
  {#if !hideUpdateDate &&
  meta.updated &&
  new Date(meta.updated).getTime() !==
    new Date(meta.published).getTime()}
    <div class="flex items-center">
      <div class="meta-icon">
        <Icon
      ...
      <div class="flex flex-row flex-nowrap items-center">
        {#if meta.tags && meta.tags.length > 0}
          {#each meta.tags as tag, i}
            {#if i > 0}
              <div class="mx-1.5 text-[var(--meta-divider)] text-sm">
                /
              </div>
            {/if}
            <a
