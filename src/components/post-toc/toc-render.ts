...
export function renderFloatingTocItem(item: TocItem) {
	const badge =
		item.badgeKind === "text"
	...
	return `<a href="#${escapeHtml(item.id)}" class="floating-toc-item" style="padding-left: ${0.5 + item.level}rem" data-level="${item.level}">${badge}<span class="floating-toc-text">${escapeHtml(item.text)}</span></a>`;
}
