...
import { permalinkConfig, profileConfig } from "../config";
 ...
 * - 构建期运行，不可在纯客户端环境使用
 */
export function resolveImageUrl(post: CollectionEntry<"posts">): ImageMetadata | undefined {
...
export async function resolvePosterImage(
  ...
  if (isLocal) {
    const normalizedPath = path
      .normalize(baseDir ? path.join("../../", baseDir, imagePath) : path.join("../../", imagePath))
      .replace(/\\/g, "/");
