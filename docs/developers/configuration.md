...
- `live2dCompanionConfig.enable` 仅提供 Live2D Companion 的默认页面挂载状态，Floating Tools 的看板娘入口仍可重新挂载组件。访客挂载、内部收起、展开位置、收起头像位置和模型索引分别写入独立 `localStorage` key；模型、expression、idle playback、iframe host、拖拽、同源 iframe header 和旧 widget 配置边界详见 [Live2D Companion 维护指南](./live2d-companion-maintenance.md)。
