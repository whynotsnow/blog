...
本项目使用本机 Codex 提示音工具在关键节点播放 macOS 系统音或 TTS。提示音只服务于注意力切换：需要维护者处理、生产部署审批、关键验证结果或任务完成。全局 Codex `Stop` hook 会在回合结束时调用 `done` 事件；项目内的 `.codex/codex-cues.json` 会覆盖全局默认设置。
...
pnpm cue play deploy-approval --mode both --preset tink --volume 3 --text "生产部署等待审批"
...
```bash
pnpm cue play done --sound /absolute/path/to/sound.mp3 --volume 2
pnpm cue set done --sound /absolute/path/to/sound.mp3 --volume 2
```

## Agent 规范

Agent 在本项目内可以使用以下命令发出明确提示：
...
详细使用规则、禁止场景和部署审批等待行为见 [`docs/agents/sound-cues.md`](../agents/sound-cues.md)。不要在本文件复制 agent 行为规范，避免两个文档漂移。
