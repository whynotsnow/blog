...
`pnpm dev:plan` 会启动相邻 `../blog.plan` sidecar 的本地预览面板，并打开规划首页；也可以通过 `pnpm dev:plan -- BLOG-0001` 直接打开指定 item 页面。自动化或仅检查 URL 时使用 `pnpm dev:plan -- --print`，只启动不打开浏览器时使用 `pnpm dev:plan -- --no-open`。`pnpm plan:status` 读取 sidecar 规划仓库，输出当前规划项和可执行状态。Sidecar 用于保存规划、决策、执行记录、验证摘要和 handoff；产品源码、部署文件、运行配置和正式项目文档仍属于当前仓库。
