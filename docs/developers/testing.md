...
- 在 CI 模式或发布前显式验证中修改依赖、lockfile、Astro/TypeScript/Playwright 配置、构建编排、影响映射或 CI 选择逻辑；
- 同时影响三个及以上互不相关的 Feature；
- 影响映射无法识别发生变化的 runtime 路径；
- 已选择的测试暴露出预期之外的跨模块依赖；
- 定时回归、手动完整验证或发布验证。

Pull Request 与普通 `main` Push 使用 CI 模式的影响映射；定时任务、手动完整验证、发布验证和 CI 中无法确定影响范围的情况仍保留完整回归作为映射遗漏的安全网。
...
- `test:affected` 根据 `tests/impact-map.json` 执行本地快速模式的受影响验证；无法分类或建议完整回归的路径会输出风险提示，但不会在本地自动执行 `verify:full`。
- `test:plan:ci` 与 `test:affected:ci` 使用 CI 模式；无法分类或高风险路径会升级到 `verify:full`。
