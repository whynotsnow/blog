...
Typography 规范化采用分批迁移。第一批覆盖首页、分类页、文章详情页、相册页、日记页，以及这些页面直接展示或条件渲染的组件链路：`src/pages/index.astro`、`src/pages/posts/`、`src/pages/albums.astro`、`src/pages/albums/`、`src/pages/diary.astro`、`src/components/home/`、`src/components/category/`、`src/components/post-detail/`、`src/components/post-toc/`、`src/components/albums/`、`src/components/diary/`、`src/components/filter-tabs/`、`PostMeta`、`MobileTOC`、`FloatingTOC`、评论入口、License、Markdown wrapper、`src/features/post-list/`、`src/features/diary/`、相册样式和 Markdown 样式。`projects`、`devices`、`friends`、`skills`、`anime`、RSS/Atom、404、播放器、设置面板等暂时按 Legacy Typography Debt 保留，只有进入后续批次时才迁移。
