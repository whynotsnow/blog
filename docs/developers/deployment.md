...
项目输出 Astro 静态站点。常规生产部署必须走 GitHub Actions `snow build CI` workflow 的手动生产发布路径，并在完整 CI 校验成功、Vercel prebuilt 产物构建并计算 digest 后，通过 `snow-base` Admin 部署审批。不要从本机、Vercel Dashboard 或平台 Git 自动部署入口直接发布生产。
