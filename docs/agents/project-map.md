...
Site notifications are Markdown content entries under `src/content/notifications`. `src/config/site-notice.ts` only owns the global enable switch, while `src/services/site-notice.ts` normalizes route visibility and builds the Activity Center view model. Astro renders notification Markdown at build time in `ActivityCenter.astro`; the Svelte runtime reads that trusted HTML from templates and owns dialog state, read state, dismissal state, and critical acknowledgement state.

Activity Center is the Navbar-level information hub mounted at the top right. `src/features/activity-center` composes notification summaries with browser-only article reading status (progress, current heading, remaining time, and a saved resume position). Its notification badge represents unread notices only; its ring represents reading progress only. Clicking a notification opens the full Markdown dialog. Notification `level` (`normal`, `important`, `urgent`, `critical`) controls ordering and interruption behavior; notification `status` (`info`, `success`, `warning`, `danger`) controls visual tone. Do not add action shortcuts here—viewport actions belong to Floating Tools.
...
Pio and Music Player configuration `enable` flags supply only the initial mounted state; they must not remove either Floating Tools entry. Visitor choices are stored separately as `pio-module-mounted` and `music-player-mounted`. Pio's outer mount contract in `src/features/pio/preferences.ts` must remain independent from the plugin's internal `.pio-hidden` / `posterGirl` state, so `pio-close` never unmounts the component. Music Player owns Audio plus Default, Mini, Expanded, and Playlist state; `src/features/music-player/events.ts` is the narrow UI contract through which Floating Tools only toggles module UI visibility and consumes explicit state/layout events. Default ↔ Mini uses one reversible morph, while Mini ↔ Expanded shares one surface geometry and easing contract in both directions. The Floating Tools switch uses keyframes generated by the same easing function and the panel transition start timestamp, so its FLIP transform stays on the same timeline in both directions. Floating Tools must not infer feature state from DOM mutations.
