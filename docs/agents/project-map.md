...
Live2D Companion and Music Player configuration `enable` flags supply only the initial mounted state; they must not remove either Floating Tools entry. Visitor choices are stored separately as `live2d-companion-mounted` and `music-player-mounted`. Live2D Companion's outer mount contract in `src/features/live2d-companion/preferences.ts` must remain independent from the internal `live2d-companion-collapsed` state, so the internal collapse control never unmounts the component. The Live2D rendering engine is isolated in `/live2d-companion/live2d-host.html`, which loads `/live2d-companion/l2d-widget.min.js` inside an iframe and receives initialization through `postMessage`; other pages and components must interact through `src/features/live2d-companion/events.ts` show, collapse, toggle, message, and expression commands instead of reaching into the iframe DOM. Its iframe shell UI consumes generic `ui` configuration and site semantic theme tokens, while model-specific data is limited to model paths, scale, offset, default parameters, expression menu metadata, model switch metadata, and avatar assets. The iframe host owns model switching and model-scoped frequent expression shortcuts with local SVG controls; keep expression actions in each model's `expressionMenu.shortcuts`, cap iframe actions at 5, and cap the full expression panel at 24 items (`4 × 6`). Idle expression playback may include shortcut expressions outside the full panel through `includeActions`; drag-ready and dragging states count as active interaction and must pause idle playback. The full expression panel is optional per model, belongs to the parent Svelte component, is fed by iframe-reported expression names, and uses a no-scroll compact grid outside iframe/canvas hit constraints. Dragging follows the same iframe boundary: the host detects the configured long-hover preparation delay, shows the configured local drag indicator, and forwards drag pointer messages, while the parent Svelte component owns position state persisted as `live2d-companion-position` and clamps it so at least 50% of the component remains recoverable within the viewport. Host theme sync must set Light/Dark state and keep widget containers/canvas transparent so the third-party widget cannot expose a white rectangle in dark mode. Do not rely on the third-party widget's remote Iconify menu buttons. `defaultParameters` are applied after model load and must not be treated as VTS ToggleExpression state management or registered as expression actions. Do not couple shell styling, controls, or command behavior to a particular Live2D model name or internal model DOM. The collapsed and first-load states use the configured circular avatar entry with loading feedback rather than a widget rest surface. Music Player owns Audio plus Default, Mini, Expanded, and Playlist state; `src/features/music-player/events.ts` is the narrow UI contract through which Floating Tools only toggles module UI visibility and consumes explicit state/layout events. Default ↔ Mini uses one reversible morph, while Mini ↔ Expanded shares one surface geometry and easing contract in both directions. The Floating Tools switch uses keyframes generated by the same easing function and the panel transition start timestamp, so its FLIP transform stays on the same timeline in both directions. Floating Tools must not infer feature state from DOM mutations.

Page modules own their placement. Home passes `HomeSupport` through `MainGridLayout`'s named `support` slot, combining the profile card with compact site stats, recent posts, categories, and tags. Category discovery starts at `/category/`, which renders the Category Hub's All Categories view; `/category/recommended/` renders the same Hub shell with score-sorted recommended posts. Concrete category pages pass `CategorySupport` through the support slot for recent posts, popular tags, and adjacent taxonomy browsing without duplicating the main filter summary. Post-detail pages pass `PostSupport` for the only desktop in-page TOC, continue-reading links, and recommended posts. Archive owns only date-oriented browsing through Calendar and Timeline; category and Tag discovery belongs to the Category Hub and concrete category pages. Site statistics are a Footer feature: `src/services/footer.ts` builds the view model from content-store metadata and `src/components/footer` renders it once. Footer height remains content-driven, with its outer, Stats, and Meta spacing owned by Footer-local tokens rather than a fixed Shell height. `PanelCard.astro` is only a visual container and must not become a placement registry.

The home and category pages own separate page compositions while sharing Post Card and Grid contracts. Home renders Recently Updated and Recommended sections with three-post limits; their section links route to `/category/` and `/category/recommended/` to favor category discovery over the Archive. The optional Technology section selects only the canonical `tech` taxonomy, ranks that category by score, caps at six posts, never backfills from another category, and disappears when empty. Category Hub pages are discovery views, not taxonomy aliases: All Categories renders category cards with recent posts and popular tags, while Recommended uses global `sortByScore()` output and does not emit or consume a category Tag JSON index. Concrete category pages remain the canonical taxonomy browsing surface: category links point to `/category/{slug}/`, and post Tag links point to the owning category with `?tag={tagSlug}`. Category pages render twelve-post pages with their taxonomy filter in the main content area. Category SSG props retain only the current page and data-free pagination metadata. Each concrete category also emits one compact `/api/categories/{slug}.json/` index. After the initial page load, Svelte prefetches that index during browser idle time only while the page is visible, online, outside `Save-Data`, and above 2g; a valid Tag query always loads immediately even when prefetch was skipped. The module cache is keyed by the complete index URL, shares promises across prefetch, Tag pagination, history, and Tag changes, and retains at most three recently used category indexes. Prefetch does not mutate component state, and request-version checks prevent an obsolete response from updating the active view. Both renderers share the semantic class contract and styles in `src/features/post-list/post-list.css`; its Card block size also owns the matching intrinsic placeholder, while cover ratio and Feed width breakpoints remain independent inline-size contracts. Category runtime URL, request state, history, filtering, and pagination behavior lives beside the category components in `src/components/category/category-page-client.ts`.

Home, category, and post-detail pages use the `container-content` strategy. Banner always remains viewport-wide, while Navbar and Main Shell share a `1280px` outer maximum width. Pages with support content use a maximum `992px` three-column Feed/main column plus a `248px–272px` support column from `1200px`, a maximum `656px` two-column Feed/main column plus support from `880px`, and a single-column stack below that. The Feed enters three columns at `932px`; the resulting Grid Cards target about `320px` while retaining `296px` as the safe minimum. Post content keeps its narrower `--width-reading` measure internally even when the main grid column is wider. Legacy pages without support center at `--width-listing`. These pages clear the legacy root-font `pageScaling`; viewport Grid rules live in `page-grid-legacy.css` and must remain scoped away from `container-content`. The shared `--navbar-shell-height` drives the Navbar outer height, Main Content offset, Page Entry clearance, and runtime scroll thresholds; Navigation controls keep their independent `44px` target. Desktop post support placement is owned by `PostSupport`: the full support shell sticks as one unit with internal scrolling for TOC, continue-reading, and recommendations. Desktop TOC shows only root headings at the document start and end, and expands only the active root branch while the reader is inside the article body. Floating Tools remains the compact/mobile TOC entry.

Page Layout Policy and Post List View Mode are separate contracts. Policy selects the responsive Shell Strategy and the page's desktop layout from configuration only; there is no user-facing Desktop Page Layout Preference. Post List View Mode changes only list-versus-grid post-list presentation and must not write or imply page layout state.
