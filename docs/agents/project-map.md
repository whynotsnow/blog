...
Mermaid fenced code blocks are transformed by the Markdown plugin pipeline before post-detail rendering. Production builds prerender initial SVG through the Playwright-based renderer in `src/plugins/mermaid-prerender.ts` and cache the SVG by Mermaid source, Mermaid version, security level, theme settings, and renderer version. `rehype-mermaid.ts` emits the inline SVG plus `data-mermaid-code` for client fallback and theme rerendering. The browser script in `src/plugins/mermaid-render-script.js` must treat prerendered SVG as already rendered and only attach sizing and zoom controls unless an explicit theme rerender is needed. Development can disable prerendering and keep client fallback behavior.
