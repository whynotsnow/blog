...
Home, category, and post-detail pages use the `container-content` strategy. Banner always remains viewport-wide, while Navbar and Main Shell use a `1400px` maximum width. The Page Shell state machine uses three Feed columns plus a `248px–272px` Sidebar from `1200px`, two Feed columns plus the same Sidebar from `880px`, full-width pre-content Widgets plus two Feed columns from `608px`, and one Feed column below that. Sidebar visibility and the two-column Feed minimum share the same width budget, so a one-column Feed can never coexist with the right Sidebar; the Supporting Row and Desktop Sidebar are also mutually exclusive. Main Grid maxima are `1352px`, `992px`, and `704px`; Feed maxima are `1064px`, `704px`, and `400px`. Multi-column Cards remain within roughly `296px–344px`, with the compact `--space-grid-gap` shared by the Shell and Feed. These pages clear the legacy root-font `pageScaling`; viewport Grid rules live in `page-grid-legacy.css` and must remain scoped away from `container-content`. The post Sidebar TOC derives its external rail from `--width-shell-wide` rather than the legacy `--page-width` formula.
