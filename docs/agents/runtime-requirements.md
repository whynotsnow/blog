...
| Manual browser validation | Visual judgment, DOM inspection, layout, network, and exploratory interaction | Controlled host Chrome |
...
- A manual Chrome result and an automated Playwright result are distinct and must be reported separately.
...
After detection, use the manual or automated lane in [workflow.md](./workflow.md#failure-handling-policy-important). Record which execution surface produced each result without copying private profile values into tracked documentation.
