...
This project uses local Codex sound cues for attention-worthy events only. Cues are for owner attention shifts: required input, production deployment approval, meaningful validation results, and task completion.
...
| `validation-start` | Process | `开始验证` | Before starting a full or long-running validation flow such as `pnpm check`, `pnpm build`, or release validation. |
...
| `done` | Completion | `任务完成` | Triggered by the global Codex `Stop` hook; agents should not trigger it frequently or manually for ordinary completion. |

Prefer concise Chinese TTS text for events that require action or report validation results, so the owner can understand the reason immediately.
